...
export function SectorRotationWidget({ onSectorExplore }: { onSectorExplore?: (sector: string) => void }) {
    const [heatmap, setHeatmap] = useState(false);
    return <Stack sx={{ height: "100%" }}>
        {heatmap ? <Grid container spacing={1} sx={{ flex: 1 }}>{sectorRows.map(([sector, strength, , , status, color]) => <Grid key={sector} size={{ xs: 6, sm: 4 }}><Tooltip title="Click to explore more" arrow><Box role="button" tabIndex={0} onClick={() => onSectorExplore?.(sector)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSectorExplore?.(sector); }} sx={{ height: "100%", minHeight: 72, p: 1.2, borderRadius: 2, bgcolor: `${color}18`, border: `1px solid ${color}45`, cursor: onSectorExplore ? "pointer" : "default", transition: "transform .18s ease", "&:hover": { transform: "translateY(-2px)" }, "&:focus-visible": { outline: `2px solid ${colors.blue}` } }}><Typography sx={{ fontWeight: 900 }}>{sector}</Typography><Typography sx={{ color, fontSize: "1.1rem", fontWeight: 900 }}>{strength}</Typography><Typography color="text.secondary" sx={{ fontSize: ".58rem" }}>{status}</Typography></Box></Tooltip></Grid>)}</Grid> :
            <Box sx={{ overflowX: "auto", flex: 1 }}><Table size="small"><TableHead><TableRow>{["Sector", "Strength", "Money Moving", "Direction", "Simple View"].map((heading) => <TableCell key={heading}>{heading}</TableCell>)}</TableRow></TableHead><TableBody>{sectorRows.map(([sector, strength, flow, trend, status, color]) => <Tooltip key={sector} title="Click to explore more" arrow><TableRow hover tabIndex={onSectorExplore ? 0 : undefined} onClick={() => onSectorExplore?.(sector)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSectorExplore?.(sector); }} sx={{ cursor: onSectorExplore ? "pointer" : "default", transition: "background-color .18s ease", "&:focus-visible": { outline: `2px solid ${colors.blue}`, outlineOffset: -2 } }}><TableCell sx={{ fontWeight: 850 }}>{sector}</TableCell><TableCell>{strength}</TableCell><TableCell sx={{ color, fontWeight: 900 }}>{flow}</TableCell><TableCell>{trend}</TableCell><TableCell><Chip size="small" label={status} sx={{ color, bgcolor: `${color}12`, border: `1px solid ${color}30` }} /></TableCell></TableRow></Tooltip>)}</TableBody></Table></Box>}
...
export function MarketBreadthWidget({ shortNumbers = false, showTooltips = true, onMetricExplore }: { shortNumbers?: boolean; showTooltips?: boolean; onMetricExplore?: (metric: string) => void }) {
    ...
    return <Grid container spacing={1}>{breadthMetrics.map(([label, value, trend, help]) => {
        const metric = <Box role={onMetricExplore ? "button" : undefined} tabIndex={onMetricExplore ? 0 : undefined} onClick={() => onMetricExplore?.(label)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onMetricExplore?.(label); }} sx={{ p: 1.05, borderRadius: 2, border: "1px solid rgba(143,161,184,.14)", bgcolor: "rgba(4,12,25,.24)", cursor: onMetricExplore ? "pointer" : "default", transition: "transform .18s ease,border-color .18s ease", "&:hover": { transform: "translateY(-2px)", borderColor: "rgba(97,114,243,.45)" }, "&:focus-visible": { outline: `2px solid ${colors.blue}` } }}><Typography color="text.secondary" noWrap sx={{ fontSize: ".55rem" }}>{label}</Typography><Stack direction="row" spacing={.55} sx={{ mt: .35, alignItems: "baseline" }}><Typography sx={{ fontWeight: 900 }}>{format(value)}</Typography><Typography sx={{ color: String(trend).startsWith("-") ? colors.red : colors.green, fontSize: ".56rem" }}>{trend}</Typography></Stack></Box>;
        return <Grid key={label} size={{ xs: 6, sm: 4, lg: 3 }}>{showTooltips ? <Tooltip title={`${help}. Click to explore more.`}>{metric}</Tooltip> : metric}</Grid>;
    })}</Grid>;
}
...
export function OpportunitiesWidget({ onScan }: { onScan?: (scan: string) => void }) {
    ...
    return <Stack spacing={1}><Grid container spacing={1.1}>{items.map(([label, count, color]) => <Grid key={label as string} size={{ xs: 12, sm: 6 }}><Tooltip title="Click to explore more" arrow><Box role="button" tabIndex={0} onClick={() => { setSelected(String(label)); onScan?.(String(label)); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { setSelected(String(label)); onScan?.(String(label)); } }} sx={{ p: 1.25, borderRadius: 2, border: "1px solid rgba(143,161,184,.15)", cursor: "pointer", transition: "transform .18s ease,border-color .18s ease", "&:hover": { transform: "translateY(-2px)", borderColor: "rgba(97,114,243,.45)" }, "&:focus-visible": { outline: `2px solid ${colors.blue}` } }}><Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center" }}><Box><Typography sx={{ fontWeight: 800, fontSize: ".72rem" }}>{label}</Typography><Typography sx={{ color, fontSize: "1.35rem", fontWeight: 900 }}>{count}</Typography></Box><Button size="small" endIcon={<ArrowForwardRoundedIcon />} onClick={(event) => { event.stopPropagation(); setSelected(String(label)); onScan?.(String(label)); }}>Scan</Button></Stack></Box></Tooltip></Grid>)}</Grid>{selected && <Typography color="primary.light" sx={{ fontSize: ".65rem" }}>{selected} selected. Opening the matching scanner view.</Typography>}</Stack>;
}

export function SmartAlertsWidget({ onAlert }: { onAlert?: (alert: string) => void }) {
    ...
    return <Stack spacing={1}>{alerts.map(([message, severity, time, color]) => <Tooltip key={message} title="Click to explore more" arrow><Stack role="button" tabIndex={0} onClick={() => onAlert?.(message)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onAlert?.(message); }} direction="row" spacing={1.1} sx={{ alignItems: "center", p: 1.1, borderRadius: 2, bgcolor: "rgba(4,12,25,.25)", border: "1px solid rgba(143,161,184,.12)", cursor: "pointer", transition: "transform .18s ease,border-color .18s ease", "&:hover": { transform: "translateX(2px)", borderColor: "rgba(97,114,243,.42)" }, "&:focus-visible": { outline: `2px solid ${colors.blue}` } }}><Box sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: color, boxShadow: `0 0 10px ${color}` }} /><Box sx={{ flex: 1 }}><Typography sx={{ fontSize: ".72rem", fontWeight: 800 }}>{message}</Typography><Typography color="text.secondary" sx={{ fontSize: ".57rem" }}>{severity} severity</Typography></Box><Typography color="text.secondary" sx={{ fontSize: ".6rem" }}>{time}</Typography></Stack></Tooltip>)}</Stack>;
}
