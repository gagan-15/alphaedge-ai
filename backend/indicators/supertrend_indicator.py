...
class SuperTrendIndicator(BaseIndicator):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for index in range(1, len(result)):
            ...
            if (
                basic_upper_band.iloc[index] < final_upper_band.iloc[previous_index]
                ...
                final_upper_band.iloc[index] = final_upper_band.iloc[previous_index]

            if (
                basic_lower_band.iloc[index] > final_lower_band.iloc[previous_index]
                or result["Close"].iloc[previous_index]
                < final_lower_band.iloc[previous_index]
            ):
                final_lower_band.iloc[index] = basic_lower_band.iloc[index]
            else:
                final_lower_band.iloc[index] = final_lower_band.iloc[previous_index]

            if index == 1:
                supertrend.iloc[index] = final_upper_band.iloc[index]

            elif (
                supertrend.iloc[previous_index] == final_upper_band.iloc[previous_index]
            ):
