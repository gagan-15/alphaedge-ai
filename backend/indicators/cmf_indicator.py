...
class CMFIndicator(BaseIndicator):
    ...
    def calculate(self, data: pd.DataFrame, period: int = 20) -> pd.DataFrame:
        """
        ...
        high_low_range = high_low_range.replace(0, pd.NA)
        ...
        rolling_money_flow = money_flow_volume.rolling(window=period).sum()

        rolling_volume = result["Volume"].rolling(window=period).sum()

        result["CMF"] = rolling_money_flow / rolling_volume

        logger.info("CMF calculation completed successfully.")
