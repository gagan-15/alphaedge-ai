...
class BollingerBandsIndicator(BaseIndicator):
    ...
    def __init__(self, period: int = 20, multiplier: int = 2):
        """
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(
            "Starting Bollinger Bands calculation " "with period=%s and multiplier=%s",
            self.period,
            self.multiplier,
        )
        ...
        result = sma_indicator.calculate(data.copy(), self.period)

        middle_band = result[f"SMA_{self.period}"]

        # Calculate Standard Deviation
        standard_deviation = data["Close"].rolling(window=self.period).std()

        # Calculate Upper and Lower Bands
        upper_band = middle_band + (standard_deviation * self.multiplier)

        lower_band = middle_band - (standard_deviation * self.multiplier)
        ...
        self.logger.info("Bollinger Bands calculation completed successfully.")
